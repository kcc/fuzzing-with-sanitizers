...
cond_next127:		
	...
	%tmp158 = xor i32 0, %tmp153		
	%tmp160 = or i32 %tmp158, 0		
...
cond_true267:		
	%tmp269 = load i8*, i8** %byteptr, align 4		
	%tmp270 = load i8, i8* %tmp269, align 1		
	%tmp270271 = zext i8 %tmp270 to i32		
	%tmp272 = getelementptr i8, i8* %tmp269, i32 1		
	store i8* %tmp272, i8** %byteptr, align 4
	%tmp276 = load i8, i8* %tmp272, align 1		
	%tmp278 = getelementptr i8, i8* %tmp269, i32 2		
	store i8* %tmp278, i8** %byteptr, align 4
	%tmp286 = icmp eq i32 %tmp270271, %markerPrefix		
	%cond = icmp eq i8 %tmp276, 0		
	%bothcond = and i1 %tmp286, %cond		
