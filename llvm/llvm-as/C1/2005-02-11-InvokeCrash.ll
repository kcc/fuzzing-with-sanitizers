...
entry:
        %tmp.7 = invoke i32 @_ZN5ArrayISt7complexIdEE8get_sizeERK10dim_vector( )
                        to label %invoke_cont.0 unwind label %cond_true.1               

invoke_cont.0:          
        %tmp.4.i = bitcast i32 %tmp.7 to i32            
        %tmp.14.0.i5 = add i32 %tmp.7, -1               
        br label %no_exit.i

no_exit.i:              
        %tmp.14.0.i.0 = phi i32 [ %tmp.14.0.i, %no_exit.i ], [ %tmp.14.0.i5, %invoke_cont.0 ]           
        %tmp.14.0.i = add i32 %tmp.14.0.i.0, -1         
        br label %no_exit.i
