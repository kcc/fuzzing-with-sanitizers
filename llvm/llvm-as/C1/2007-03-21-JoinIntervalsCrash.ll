...
entry:
	%spec.1961 = zext i32 %spec.1 to i64		
	%spec.1961.adj = shl i64 %spec.1961, 32		
	%spec.1961.adj.ins = or i64 %spec.1961.adj, 0		
	%tmp10959 = lshr i64 %spec.1961.adj.ins, 32		
	%tmp1920 = inttoptr i64 %tmp10959 to %struct.tree_common*		
	%tmp21 = getelementptr %struct.tree_common, %struct.tree_common* %tmp1920, i32 0, i32 3		
	%tmp2122 = bitcast i8* %tmp21 to i32*		
...
bb866:		
	%tmp874 = load i32, i32* %tmp2122		
	%tmp876877 = trunc i32 %tmp874 to i8		
	icmp eq i8 %tmp876877, 1		
...
cond_next881:		
	%tmp884885 = inttoptr i64 %tmp10959 to %struct.tree_identifier*		
	%tmp887 = getelementptr %struct.tree_identifier, %struct.tree_identifier* %tmp884885, i32 0, i32 1, i32 0		
	%tmp888 = load i8*, i8** %tmp887		
	tail call void (i32, ...) @error( i32 undef, i8* %tmp888 )
	ret void

cond_true918:		
	%tmp920957 = trunc i64 %spec.1961.adj.ins to i32		
	ret void
}
