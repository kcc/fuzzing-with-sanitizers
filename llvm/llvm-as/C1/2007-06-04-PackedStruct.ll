...
@someMap = global %"struct.std::map<int,int,std::less<int>,std::allocator<std::pair<const int, int> > >" zeroinitializer		
@llvm.global_ctors = appending global [1 x { i32, void ()* }] [ { i32, void ()* } { i32 65535, void ()* @_GLOBAL__I_someMap } ]		
@llvm.global_dtors = appending global [1 x { i32, void ()* }] [ { i32, void ()* } { i32 65535, void ()* @_GLOBAL__D_someMap } ]		
...
entry:
	%tmp1 = icmp eq i32 %__priority, 65535		
	%tmp4 = icmp eq i32 %__initialize_p, 1		
	%tmp7 = and i1 %tmp1, %tmp4		
