

target datalayout = "e-p:32:32:32-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:32:64-f32:32:32-f64:32:64-v64:64:64-v128:128:128-a0:0:64-f80:32:32"
target triple = "i386-pc-linux-gnu"
@p = weak global i8* null		

define i32 @main() nounwind  {
entry:
	%tmp248 = call i8* @llvm.stacksave( )		
	%tmp2752 = alloca i32		
	%tmpcast53 = bitcast i32* %tmp2752 to i8*		
	store i32 2, i32* %tmp2752, align 4
	store volatile i8* %tmpcast53, i8** @p, align 4
	br label %bb44

bb:		
	ret i32 0

bb44:		
	%indvar = phi i32 [ 0, %entry ], [ %tmp3857, %bb44 ]		
	%tmp249 = phi i8* [ %tmp248, %entry ], [ %tmp2, %bb44 ]		
	%tmp3857 = add i32 %indvar, 1		
	call void @llvm.stackrestore( i8* %tmp249 )
	%tmp2 = call i8* @llvm.stacksave( )		
	%tmp4 = srem i32 %tmp3857, 1000		
	%tmp5 = add i32 %tmp4, 1		
	%tmp27 = alloca i32, i32 %tmp5		
	%tmpcast = bitcast i32* %tmp27 to i8*		
	store i32 1, i32* %tmp27, align 4
	%tmp34 = getelementptr i32, i32* %tmp27, i32 %tmp4		
	store i32 2, i32* %tmp34, align 4
	store volatile i8* %tmpcast, i8** @p, align 4
	%exitcond = icmp eq i32 %tmp3857, 999999		
	br i1 %exitcond, label %bb, label %bb44
}

declare i8* @llvm.stacksave() nounwind 

declare void @llvm.stackrestore(i8*) nounwind 
