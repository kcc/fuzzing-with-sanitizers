

define i32 @seli32_1(i32 %a) {
entry:
	%tmp.1 = icmp slt i32 %a, 0		
	%retval = select i1 %tmp.1, i32 5, i32 0		
	ret i32 %retval
}

define i32 @seli32_2(i32 %a, i32 %b) {
entry:
	%tmp.1 = icmp slt i32 %a, 0		
	%retval = select i1 %tmp.1, i32 %b, i32 0		
	ret i32 %retval
}

define i32 @seli32_3(i32 %a, i16 %b) {
entry:
	%tmp.2 = sext i16 %b to i32		
	%tmp.1 = icmp slt i32 %a, 0		
	%retval = select i1 %tmp.1, i32 %tmp.2, i32 0		
	ret i32 %retval
}

define i32 @seli32_4(i32 %a, i16 %b) {
entry:
	%tmp.2 = zext i16 %b to i32		
	%tmp.1 = icmp slt i32 %a, 0		
	%retval = select i1 %tmp.1, i32 %tmp.2, i32 0		
	ret i32 %retval
}

define i16 @seli16_1(i16 %a) {
entry:
	%tmp.1 = icmp slt i16 %a, 0		
	%retval = select i1 %tmp.1, i16 7, i16 0		
	ret i16 %retval
}

define i16 @seli16_2(i32 %a, i16 %b) {
	%tmp.1 = icmp slt i32 %a, 0		
	%retval = select i1 %tmp.1, i16 %b, i16 0		
	ret i16 %retval
}

define i32 @seli32_a_a(i32 %a) {
	%tmp = icmp slt i32 %a, 1		
	%min = select i1 %tmp, i32 %a, i32 0		
	ret i32 %min
}
