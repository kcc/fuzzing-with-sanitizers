...
@llvm.used = appending global [1 x i8*] [ i8* bitcast (i16 (%struct.JPEGGlobals*)* @ExtractBufferedBlocksIgnored to i8*) ], section "llvm.metadata"		

define signext i16 @ExtractBufferedBlocksIgnored(%struct.JPEGGlobals* %globp)  nounwind {
entry:
	%tmp4311 = getelementptr %struct.JPEGGlobals, %struct.JPEGGlobals* %globp, i32 0, i32 70		
	%tmp4412 = load i32, i32* %tmp4311, align 16		
	%tmp4613 = icmp sgt i32 %tmp4412, 0		
	br i1 %tmp4613, label %bb, label %bb49

bb:		
	%component.09 = phi i16 [ 0, %entry ], [ %tmp37, %bb28 ]		
	%tmp12 = sext i16 %component.09 to i32		
	%tmp6 = getelementptr %struct.JPEGGlobals, %struct.JPEGGlobals* %globp, i32 0, i32 77, i32 %tmp12		
	%tmp7 = load i16*, i16** %tmp6, align 4		
	%tmp235 = getelementptr %struct.JPEGGlobals, %struct.JPEGGlobals* %globp, i32 0, i32 71, i32 %tmp12		
	%tmp246 = load i32, i32* %tmp235, align 4		
	%tmp267 = icmp sgt i32 %tmp246, 0		
	br i1 %tmp267, label %bb8, label %bb28

bb8:		
	%indvar = phi i32 [ 0, %bb ], [ %indvar.next2, %bb8 ]		
	%theDCTBufferIter.01.rec = shl i32 %indvar, 6		
	%tmp10.rec = add i32 %theDCTBufferIter.01.rec, 64		
	%tmp10 = getelementptr i16, i16* %tmp7, i32 %tmp10.rec		
	%i.02 = trunc i32 %indvar to i16		
	%tmp13 = add i16 %i.02, 1		
	%phitmp = sext i16 %tmp13 to i32		
	%tmp26 = icmp slt i32 %phitmp, %tmp246		
	%indvar.next2 = add i32 %indvar, 1		
	br i1 %tmp26, label %bb8, label %bb28

bb28:		
	%theDCTBufferIter.0.lcssa = phi i16* [ %tmp7, %bb ], [ %tmp10, %bb8 ]		
	store i16* %theDCTBufferIter.0.lcssa, i16** %tmp6, align 4
	%tmp37 = add i16 %component.09, 1		
	%phitmp15 = sext i16 %tmp37 to i32		
	%tmp46 = icmp slt i32 %phitmp15, 42		
