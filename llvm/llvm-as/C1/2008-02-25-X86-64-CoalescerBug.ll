...
bb53:		
	...
	%tmp2728128 = load %struct.XX*, %struct.XX** null		
	%tmp61 = load %struct.YY*, %struct.YY** %tmp55, align 8		
	%tmp62 = getelementptr %struct.YY, %struct.YY* %tmp61, i32 0, i32 0		
	%tmp63 = load i64, i64* %tmp62, align 8		
	%tmp6566 = zext i16 %tmp45 to i64		
	%tmp67 = shl i64 %tmp6566, 1		
	call void @BB( %struct.YY** %tmp55, i64 %tmp67, i8 signext  0, %struct.XX* %uen )
	%tmp121131 = icmp eq i16 %tmp45, 1		
...
bb70:		
	%indvar133 = phi i32 [ %indvar.next134, %bb119 ], [ 0, %bb70.preheader ]		
	%tmp.135 = trunc i64 %tmp63 to i32		
	%tmp136 = shl i32 %indvar133, 1		
	%DD = add i32 %tmp136, %tmp.135		
	%tmp73 = load %struct.ZZ**, %struct.ZZ*** %tmp72, align 8		
	br i1 false, label %bb119, label %bb77

bb77:		
	%tmp8384 = trunc i32 %DD to i16		
	%tmp85 = sub i16 0, %tmp8384		
	store i16 %tmp85, i16* null, align 8
	call void @CC( %struct.YY** %tmp55, i64 0, i64 2, i8* null, %struct.XX* %uen )
	ret i8 0

bb119:		
	%indvar.next134 = add i32 %indvar133, 1		
	br label %bb70
