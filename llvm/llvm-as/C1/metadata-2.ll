	...
	%0 = type { %object.ModuleInfo.__vtbl*, i8*, %"byte[]", %1, %"ClassInfo[]", i32, void ()*, void ()*, void ()*, i8*, void ()* }		
	%1 = type { i64, %object.ModuleInfo* }		
	%2 = type { i32, void ()* }		
	...
	%"void*[]" = type { i64, i8** }
@_D10ModuleInfo6__vtblZ = external constant %object.ModuleInfo.__vtbl		
@.str = internal constant [20 x i8] c"tango.core.BitManip\00"		
@_D5tango4core8BitManip8__ModuleZ = global %0 { %object.ModuleInfo.__vtbl* @_D10ModuleInfo6__vtblZ, i8* null, %"byte[]" { i64 19, i8* getelementptr ([20 x i8], [20 x i8]* @.str, i32 0, i32 0) }, %1 zeroinitializer, %"ClassInfo[]" zeroinitializer, i32 4, void ()* null, void ()* null, void ()* null, i8* null, void ()* null }		
@_D5tango4core8BitManip11__moduleRefZ = internal global %ModuleReference { %ModuleReference* null, %object.ModuleInfo* bitcast (%0* @_D5tango4core8BitManip8__ModuleZ to %object.ModuleInfo*) }		
@_Dmodule_ref = external global %ModuleReference*		
@llvm.global_ctors = appending constant [1 x %2] [%2 { i32 65535, void ()* @_D5tango4core8BitManip16__moduleinfoCtorZ }]		

define fastcc i32 @_D5tango4core8BitManip6popcntFkZi(i32 %x_arg) nounwind readnone {
entry:
	%tmp1 = lshr i32 %x_arg, 1		
	%tmp2 = and i32 %tmp1, 1431655765		
	%tmp4 = sub i32 %x_arg, %tmp2		
	%tmp6 = lshr i32 %tmp4, 2		
	%tmp7 = and i32 %tmp6, 858993459		
	%tmp9 = and i32 %tmp4, 858993459		
	%tmp10 = add i32 %tmp7, %tmp9		
	%tmp12 = lshr i32 %tmp10, 4		
	%tmp14 = add i32 %tmp12, %tmp10		
	%tmp16 = and i32 %tmp14, 252645135		
	%tmp18 = lshr i32 %tmp16, 8		
	%tmp20 = add i32 %tmp18, %tmp16		
	%tmp22 = and i32 %tmp20, 16711935		
	%tmp24 = lshr i32 %tmp22, 16		
	%tmp26 = add i32 %tmp24, %tmp22		
	%tmp28 = and i32 %tmp26, 65535		
	ret i32 %tmp28
}
...
entry:
	%tmp1 = lshr i32 %x_arg, 1		
	%tmp2 = and i32 %tmp1, 1431655765		
	%tmp4 = shl i32 %x_arg, 1		
	%tmp5 = and i32 %tmp4, -1431655766		
	%tmp6 = or i32 %tmp2, %tmp5		
	%tmp8 = lshr i32 %tmp6, 2		
	%tmp9 = and i32 %tmp8, 858993459		
	%tmp11 = shl i32 %tmp6, 2		
	%tmp12 = and i32 %tmp11, -858993460		
	%tmp13 = or i32 %tmp9, %tmp12		
	%tmp15 = lshr i32 %tmp13, 4		
	%tmp16 = and i32 %tmp15, 252645135		
	%tmp18 = shl i32 %tmp13, 4		
	%tmp19 = and i32 %tmp18, -252645136		
	%tmp20 = or i32 %tmp16, %tmp19		
	%tmp22 = lshr i32 %tmp20, 8		
	%tmp23 = and i32 %tmp22, 16711935		
	%tmp25 = shl i32 %tmp20, 8		
	%tmp26 = and i32 %tmp25, -16711936		
	%tmp27 = or i32 %tmp23, %tmp26		
	%tmp29 = lshr i32 %tmp27, 16		
	%tmp31 = shl i32 %tmp27, 16		
	%tmp32 = or i32 %tmp29, %tmp31		
	ret i32 %tmp32
}

define internal void @_D5tango4core8BitManip16__moduleinfoCtorZ() nounwind {
moduleinfoCtorEntry:
	%current = load %ModuleReference*, %ModuleReference** @_Dmodule_ref		
